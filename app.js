...
app.get('/camps',function(req,res)
{
    ...
    {
        res.render('index',{camps:camps,currentUser:req.user});
    })
});

//NEW CAMP FORM (GET)
app.get('/camps/new',isLoggedIn,function(req,res){
    res.render('newCamp',{error:""})
})
...
app.post('/camps',isLoggedIn,function(req,res)
{
    if(req.body.camp.name==""){
        res.render("newCamp",{error:"<strong>X  Error! </strong> Name not entered."})
    }
    else{
...
app.get('/camps/:id/edit',function(req,res){
    CG.findById(req.params.id,function(err,camp){
        if(camp.author.username==req.user.username){
            res.render("editCamp",{camp:camp,error:""})
        }
        else{
...
app.put('/camps/:id',function(req,res){
    if(req.body.camp.name==""){
        res.render("editCamp",{camp:req.body.camp,error:"<strong>X   Error! </strong> Name not entered."})
    }
    else{
        CG.findById(req.params.id,function(err,camp){
            if(camp.author.username==req.user.username){
                CG.findOneAndUpdate({_id:req.params.id},req.body.camp,function(err,camp){
                    res.redirect('/camps/'+req.params.id)
                })
            }
            else{
...
app.delete('/camps/:id',function(req,res){
    CG.findById(req.params.id,function(err,camp){
        if(camp.author.username==req.user.username){
            CG.findOneAndDelete({_id:req.params.id},function(err,camp){
                res.redirect('/camps')
            })
        }
        else{
...
app.get("/camps/:campid/comments/:commentid",function(req,res){
    CG.findById(req.params.campid,function(err,camp){
            ...
            Comment.findById(req.params.commentid,function(err,comment){
                ...
                // camp.save();
                Comment.findOneAndDelete({_id:req.params.commentid},function(err,comm){
